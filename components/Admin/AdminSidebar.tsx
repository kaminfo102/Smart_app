...
import { LayoutDashboard, Users, ShoppingBag, LogOut, X, Menu, BookOpen, HelpCircle, Image, Calendar, ArrowRight, Settings, Handshake, Megaphone } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

interface AdminSidebarProps {
  activeTab: 'dashboard' | 'users' | 'orders' | 'lms' | 'hero' | 'events' | 'cooperation' | 'promotions' | 'help' | 'settings';
  setActiveTab: (tab: 'dashboard' | 'users' | 'orders' | 'lms' | 'hero' | 'events' | 'cooperation' | 'promotions' | 'help' | 'settings') => void;
  isOpen: boolean;
...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ activeTab, setActiveTab, isOpen, setIsOpen, onLogout }) => {
  ...
  const menuItems = [
    ...
    { id: 'promotions', label: 'دیالوگ‌های تبلیغاتی', icon: Megaphone },
