...
import { Save, AlertCircle, Database, Globe, Key, Lock } from 'lucide-react';
...
const Settings: React.FC = () => {
  ...
  if (!user || !user.roles?.includes('administrator')) {
      return (
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center space-y-4">
              <div className="bg-red-100 dark:bg-red-900/20 p-6 rounded-full text-red-600">
                  <Lock className="w-12 h-12" />
              </div>
              <h1 className="text-xl font-bold">عدم دسترسی</h1>
              <p className="text-gray-500">شما مجوز دسترسی به این صفحه را ندارید.</p>
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
        window.location.href = '/'; // Hard reload to reset services
    }, 1000);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-3 mb-8">
        <div className="bg-primary-100 dark:bg-primary-900 p-3 rounded-2xl text-primary-600 dark:text-primary-400">
             <Database className="w-8 h-8" />
        </div>
        <div>
            <h1 className="text-2xl font-bold">تنظیمات اتصال</h1>
            <p className="text-gray-500 text-sm">مدیریت ارتباط با وردپرس (مخصوص مدیر)</p>
        </div>
      </div>
